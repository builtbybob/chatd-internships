...
**Important**: The root directory should not contain `docker-compose.yml` or `.env` files in production. These are environment-specific and generated by the setup scripts. This design prevents git conflicts when updating deployments and allows multiple isolated environments.
