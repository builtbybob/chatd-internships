...
# Abort change detection if storage is empty to prevent duplicate insertions (recommended: true)
# Set to false only for initial setup or intentional fresh start
...
echo "2. Start the environment: sudo $ENV_NAME start"
echo "3. Enable auto-start: sudo $ENV_NAME enable"
echo "4. Check status: sudo $ENV_NAME status"
echo ""
